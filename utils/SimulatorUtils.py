...
# calculate the nearest neighbour of the critiqued query
# still need to validate, TODO
def rank_by_nn(keyphrase_embeddings, query_index, **unused):
    '''
    Args:
        keyphrase_embeddings: the KAV embeddings of the keyphrases in the corpus
        query_index: the current keyphrase query being critiqued

    Returns:
        rank of items, descending accoridng to similarity level with the input query
    '''

    cos_sim = cosine_similarity(keyphrase_embeddings, keyphrase_embeddings[query_index][np.newaxis,:])
    candidate_index = np.array(list(range(keyphrase_embeddings.shape[0])))
    rank = candidate_index[cos_sim.flatten().argsort()[::-1]]

    return rank


# calculate and finds the item ranking for critiquing TODO
def rank_by_distributional(keyphrase_hierarchies, query_index, **unused):
    '''
    Args:
        keyphrase_hierarchies: this is the object, we'll use the node dictionary for now
        query_index: the original query being critiqued
        **unused:
    Returns:
        the parent node
    '''
    #rule for now is to push back the node's parent and children nodes
    if not keyphrase_hierarchies.node_dict[query_index].parent is None:
        parent_node = np.array([keyphrase_hierarchies.node_dict[query_index].parent.name])
    else:
        parent_node = np.array([])

    children_nodes = np.array([child_node.name for child_node in keyphrase_hierarchies.node_dict[query_index].children])

    return np.concatenate((parent_node, children_nodes, np.array([query_index])), axis=None)
...
def update_posterior_logistic(x, y, mu_0, S_0, prec_y, alpha):
    ...
    for i in range(100000):
        loss = - (-0.5 * (mu_1 - mu_0).T @ S_0_inv @ (mu_1 - mu_0) + \
                  torch.tensor(y) * torch.log(torch.sigmoid(alpha * mu_1.T @ x)) + (torch.tensor(1.0 - y)) * torch.log(
                    1.0 - torch.sigmoid(alpha * mu_1.T @ x))).squeeze()
        # print(loss)
...
get_clarification_query_func = {
    'none': None,
    'random': rank_by_random, #rank_by_pop
    'NN': rank_by_nn,
    'distributional': rank_by_distributional,
    'distributionalNoUserEmbed': rank_by_distributional
}