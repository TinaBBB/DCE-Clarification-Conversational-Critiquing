...
def gaussian_nll(mu, sigma, x):
    ...
    se = 0.5 * torch.sum(torch.pow((x - mu), 2)) / (len(mu.nonzero()) * (2 * torch.pow(sigma, 2))) + torch.log(sigma)
    return se / (len(mu))
...
# computes the log of the summation of exponential terms
# using the Log-Sum-Exp Trick https://gregorygundersen.com/blog/2020/02/09/log-sum-exp/
# def logsumexp(pos_x, neg_x, num_pos_samples, num_neg_samples, pos_coefficients, neg_coefficients):
#     # pos_x : [(batch_size * m) * 1]
#     # neg_x : [(batch_size * n) * 1]
#     # for the same anchoring point, concatenating samples horizontally
#     # concat = torch.cat((pos_x.reshape(-1, num_pos_samples), neg_x.reshape(-1, num_neg_samples)), 1).detach()
#
#     # pos_x: [(batch_size * m) * 1], reshaped neg_x: [(batch_size * m) * n]
#     input_shape = pos_x.shape
#     pos_x = pos_x.reshape(-1, 1)
#     neg_x = torch.repeat_interleave(neg_x.reshape(-1, num_neg_samples), repeats=num_pos_samples, dim=0)
#     concat = torch.cat((pos_x, neg_x), 1).detach()
#
#     # find the maximum sampling point for each positive-anchor point [(batch_size * m) * 1]
#     c, _ = torch.max(concat, dim=1, keepdim=True)
#
#     # c_repeat = torch.ravel(torch.repeat_interleave(c, repeats=num_samples, dim=0))
#     # assert c_repeat.shape == pos_x.shape
#     # assert c_repeat.shape == neg_x.shape
#
#     # coeff * exp(x) = exp(x + log(coeff))
#
#     pos_x_exp = torch.exp(pos_x + np.log(pos_coefficients) - c)
#     neg_x_exp = torch.exp(neg_x + np.log(neg_coefficients) - c)
#
#     # for each anchoring point, sum over each positive sample and each negative sample
#     # repeat each vertically for each sample in the mini-batch
#     # sum_pos_uk_kernel = torch.repeat_interleave(
#     #     torch.sum(pos_x_exp.reshape(-1, num_samples), axis=1), repeats=num_samples, dim=0)
#     sum_neg_uk_kernel = torch.sum(neg_x_exp, axis=1).reshape(-1, 1)
#
#     assert sum_neg_uk_kernel.shape == pos_x.shape
#     # assert sum_pos_uk_kernel.shape == pos_x.shape
#
#     return (c + torch.log(pos_x_exp + sum_neg_uk_kernel)).reshape(input_shape)  # summing of the denominator
...
#     # for each anchor point in the batch
#     for i in range(pos_entries.shape[0]):
#         # negative entries would be the ones that are not positive and not in the test set.
#         if test_pos_entries is not None:
#             neg_entry = list(set(range(matrix.shape[1])) - set(pos_entries[i]) - set(test_pos_entries[i]))
#             # neg_entry = list(set(range(matrix.shape[1])) - set(pos_entries[i]))
#         else:
#             neg_entry = list(set(range(matrix.shape[1])) - set(pos_entries[i]))
#
#         if avoid_self_sample:  # keyphrase case
#             if idx[i] in neg_entry:
#                 neg_entry.remove(idx[i])
#             elif idx[i] in pos_entries[i]:  # not likely to happen, but double shield here
#                 pos_entries[i].remove(idx[i])
#
#             # use kk_ppmi for distributional probabilities
#             # pos_word_prob = kk_pmi[idx[i], pos_entries[i]].toarray().ravel()
#             pos_word_prob = matrix[idx[i]].data
#         # user case, use log frequency of uk
#         else:
#             pos_word_prob = matrix[idx[i]].data
#
#         pos_word_prob = pos_word_prob / pos_word_prob.sum(axis=0)  # normalize
#
#         assert abs(pos_word_prob.sum() - 1) < 1e-4
#
#         pos = np.random.choice(pos_entries[i], sample_number, p=pos_word_prob).tolist()
#
#         # Need to normalize the sampling probability too
#         neg_word_prob = word_prob[neg_entry].ravel()  # locate negative entries
#         neg_word_prob = neg_word_prob / neg_word_prob.sum(axis=0)  # normalize
#
#         assert abs(neg_word_prob.sum() - 1) < 1e-4
#
#         neg = np.random.choice(neg_entry, sample_number, p=neg_word_prob).tolist()
#         # neg = np.random.choice(neg_entry, sample_number).tolist()
#
#         pos_samples.append(pos)
#         neg_samples.append(neg)

# def simple_distribution_sample():
#     # for each anchor point in the batch
#     for i in range(pos_entries.shape[0]):
#         # negative entries would be the ones that are not positive and not in the test set.
#         # if test_pos_entries is not None:
#         # neg_entry = list(set(range(matrix.shape[1])) - set(pos_entries[i]))
#
#         # if avoid_self_sample and idx[i] in pos_entries[i]:  # keyphrase case
#         #     pos_entries[i].remove(idx[i])
#             # if idx[i] in neg_entry:
#             #     neg_entry.remove(idx[i])
#             # elif idx[i] in pos_entries[i]:  # not likely to happen, but double shield here
...
#         assert abs(pos_word_prob.sum() - 1) < 1e-4
#
#         pos = np.random.choice(pos_entries[i], uk_pos_num, p=pos_word_prob).tolist()
#
#         neg_entry = list(set(range(matrix.shape[1])) - set(pos))
#         # if avoid_self_sample and idx[i] in neg_entry:
#         #     neg_entry.remove(idx[i])
#
#         neg = np.random.choice(neg_entry, uk_neg_num).tolist()
#         pos_samples.append(pos)
